...
function slugify(s: string) {
  return s.replace(/\s+/g, '-').replace(/[^\w\-./]/g, '').slice(0, 30);
}

function buildSubId(u: URL) {
  const parts = [
    slugify(u.searchParams.get('country') || ''),
    slugify(u.searchParams.get('city') || ''),
    slugify(u.searchParams.get('from') || ''),
    slugify(u.searchParams.get('to') || ''),
    slugify(u.searchParams.get('placement') || '')
  ].filter(Boolean);
